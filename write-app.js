    ...
    <>
      <nav className={\`navbar \${isScrolled ? 'scrolled' : ''}\`}>
        <div className="container navbar-container">
          ...
          <div className={\`nav-links \${isMobileMenuOpen ? 'open' : ''}\`}>
            ...
            <button className="lang-toggle" onClick={() => setLang(lang === 'tr' ? 'en' : 'tr')} style={{ background: 'none', border: '1px solid currentColor', borderRadius: '20px', padding: '6px 14px', color: 'inherit', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', fontWeight: 'bold', marginLeft: '1rem' }}>
      ...
      <section id="resume" className="section resume" style={{ backgroundColor: '#F8FAFC' }}>
        <div className="container">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={fadeUpVariant} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '3rem' }}>
            ...
            <a href={\`\${import.meta.env.BASE_URL}C_Balki_Gemirter_Alacam_CV.pdf\`} download="C_Balki_Gemirter_Alacam_CV.pdf" className="btn btn-primary" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', marginTop: '1rem' }}>
              <Download size={20} />
      ...
      <footer className="footer">
        <div className="container footer-container">
          <p>&copy; {new Date().getFullYear()} C. Balkı GEMİRTER ALAÇAM. All rights reserved.</p>
        </div>
      </footer>
    </>
  );
